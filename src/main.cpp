...
#if defined(NDEBUG)
# error "CNinecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0afe84b2cba6312826dbe2ea90b500fd3958fb135cebc5d81c11bbe1fc7264b2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CNinecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CNinecoin Signed Message:\n";
...
{
    // CNinecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // CNinecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // CNinecoin: 3.5 days
static const int64 nTargetSpacing = 3.0 * 60; // CNinecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CNinecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CNinecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3833e982ce1ce876033c53d8c0dc12df44fd888205c88346d26d7828278d2b75");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NRA member, students call for change in gun laws";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 35 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fd9d482de82c5530ff15572fcc585ac56bd33f6070f1215e3c966d1bdbb659e5e9ca843af60e026a3747246826debba9ba783cd44773185ba0b62974d79bf8a3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519221899;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085369916;

        if (fTestNet)
        {
            block.nTime    = 1519221884;
            block.nNonce   = 385491150;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd1be3eb7ca05fc4602d5fb1f4afd73c7b3f4ec4d24034edf62a107a6dde78a9b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbe, 0xdf }; // CNinecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CNinecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CNinecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CNinecoinMiner(CWallet *pwallet)
{
    printf("CNinecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cninecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CNinecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CNinecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CNinecoinMiner, pwallet));
}
